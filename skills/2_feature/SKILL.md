...
1. If this is a new session and project context is not yet loaded, invoke the `0_prime` skill before continuing
   - 0_prime loads project context and `# WORKSPACE` rules automatically
2. Use context loaded by 0_prime for the research phase
...
Invoke the `4_define_test_case` skill with the feature name to draft acceptance test cases in DSL format covering happy paths, edge cases, error scenarios, and auth.
