# 1_locate-code

> Find WHERE code lives for a feature or topic — fast, without reading file contents.

---

## What it does

`1_locate-code` maps the filesystem for a given concept and returns file paths grouped by layer. It deliberately stops at locations — no content analysis — so it runs fast and gives you a clean map to orient before diving deeper.

It searches across:
- `app/` — controllers, models, jobs
- `lib/` — services, clients, utilities
- `spec/` — test files mirroring source paths
- `config/` — configuration files
- `docs/` — documentation

Results are grouped by purpose so you know not just where something is, but what role each file plays.

---

## When to use

- Orienting in an unfamiliar codebase or feature area
- As the first step before running `1_analyze-code`
- When `0_prime` invokes it during the Research phase
- Finding all files related to a concept before refactoring

---

## Install

```bash
npx skills add pdkproitf/skills@1_locate-code
```

---

## Usage

**Claude Code:**
```
/1_locate-code user authentication
/1_locate-code video publishing
/1_locate-code PaymentProcessor
```

**Other tools:**
```
@1_locate-code <feature name, topic, or concept>
```

---

## Output

```
## File Locations for [topic]

### Implementation Files
- `app/services/feature.rb` — one-line purpose

### Test Files
- `spec/services/feature_spec.rb` — what it tests

### Configuration
- `config/feature.yml` — what it configures

### Related Directories
- `lib/feature/` — contains X files

### Entry Points
- `app/controllers/feature_controller.rb` — where the topic is wired in
```
