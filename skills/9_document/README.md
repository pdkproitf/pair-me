...
`9_document` analyzes `git diff origin/main` to understand what was built, optionally reads the original spec for requirements context, and writes a structured markdown doc to `docs/core/`. It also updates `context_dictionary.md` — a registry that tells the AI when to load each doc based on keywords, so documentation surfaces automatically when relevant.
...
/9_document
/9_document adw-42
/9_document adw-42 docs/specs/1711234567-feature-adw-42-add-retry.md
/9_document adw-42 docs/specs/feature.md ~/Desktop/screenshots
```

**Other tools:**
```
@9_document [adw_id] [spec_path] [screenshots_dir]
```
