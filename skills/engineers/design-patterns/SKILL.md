...
- `feature` — **always**, in `design` mode, during its Step 3 (Design the Code Structure). It settles the high-level structure: components, boundaries, seams. Surviving patterns are recorded under **Conventions to Follow** as rules; "no pattern" is a valid and expected outcome, and the rejected candidates are part of the deliverable.
- `implement` — in `apply` mode, at the **start** of each phase (its Step 4.1), when that phase realizes a pattern the spec recorded or a boundary turns out different on disk than the plan assumed. Invoked before any code is written, never as a mid-implementation rescue.
