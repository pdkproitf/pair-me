...
`9_commit` analyzes a git diff, groups file changes by logical concern, and produces a properly formatted [Conventional Commits 1.0.0](https://www.conventionalcommits.org/) message for each group. If asked to commit, it executes the commits sequentially after confirmation.
...
- After completing a phase in `3_implement`
- At the end of any coding session before pushing
- When `5_save_progress` needs to commit WIP changes
