...
`3_implement` reads a spec file produced by `2_feature` and executes it end to end. It follows the plan exactly, adapts only when the codebase has evolved since the spec was written, and surfaces mismatches rather than silently deviating.
...
5. Commits the phase using the `9_commit` skill
...
- Executing a plan created by `2_feature`
- Resuming interrupted implementation (works from checkboxes)
- When `6_resume_work` identifies the next unchecked spec step
...
**Claude Code:**
```
/3_implement docs/specs/1711234567-feature-add-retry-logic.md
```

**Other tools:**
```
@3_implement <path to spec file>
```
...
- Reads spec files written by `2_feature`
- Invokes `9_commit` for each completed phase
